...
export default function MealItemAddModal({ modalId, show }: { modalId: string, show?: boolean }) {
    const [quantity, setQuantity] = useState(undefined as number | undefined);
    ...
    const increment = () => setQuantity((old) => (old ?? 0) + 1)
    const decrement = () => setQuantity((old) => Math.max(0, (old ?? 0) - 1))
            ...
            <dialog id={modalId} className="modal modal-bottom sm:modal-middle">
                <form method="dialog" className="modal-box bg-gray-800 text-white">
                    ...
                    <div className="flex mt-3">
                        <input
                            value={quantity}
                            onChange={(e) => setQuantity(parseInt(e.target.value.replace(/[^0-9]/, '')) || undefined)}
