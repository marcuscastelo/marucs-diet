...
export default function Meal({ mealData, onNewItem, onEditItem, onUpdateMeal, className, locked }: MealProps) {
    ...
    const handleCopyMeal = (e: React.MouseEvent) => {
        e.preventDefault();
        if (locked) {
            alert('Não é possível copiar uma refeição bloqueada, desbloqueie-a primeiro.');
            return;
        }
    ...
    const handlePasteMeal = (e: React.MouseEvent) => {
        e.preventDefault();
        if (locked) {
            alert('Não é possível colar uma refeição bloqueada, desbloqueie-a primeiro.');
            return;
        }
                    ...
                    >
                        <div className={`bg-gray-800 p-3 ${className || ''}`}>
                            <div className="flex flex-col 2xs:flex-row">
                                ...
                                <div className={`ml-auto flex gap-2 ${locked && 'hidden'}`}>
                                    <div
